...
function NewsDownloader:processFeed(feed_type, feeds, cookies, limit, download_full_article, include_images, message, enable_filter, filter_element)
    ...
    for index, feed in pairs(feed_item) do
        ...
        if feed_type == FEED_TYPE_RSS then
            ...
            feed.description = feed.description and feed.description[1] or feed.description or "" --- @todo This should select the one with type="html" if there is a choice.
        ...
        elseif feed_type == FEED_TYPE_ATOM then
            ...
            feed_description = feed.content and feed.content[1] or feed.content or "" --- @todo This should select the one with type="html" if there is a choice.
        else
            feed_title = feed.title and feed.title[1] or feed.title
            feed_description = feed.summary or ""
        end
